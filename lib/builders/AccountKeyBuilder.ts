...
export class AccountKeyBuilder {
    ...
    private isCOSEPublicKeyEC2(cosePublicKey: COSEPublicKey): cosePublicKey is COSEPublicKeyEC2 {
    ...
    private decodeCredentialPublicKey(publicKey: Uint8Array): COSEPublicKey {
        ...
        return _decodeCredentialPublicKeyInternals.stubThis(this.decodeCborFirstItem<COSEPublicKey>(publicKey));
    }

    encodeCall(contractInterface: ethers.Interface, functionName: string, args: any[]): string {
        // 함수 시그니처와 인자를 인코딩
    ...
    decodeZkOAuthRS256KeyInitData(encodedData: string): {
        ...
        const contractInterface = new ethers.Interface(AccountKeyZkOAuthRS256VerifierABI);
        ...
        const [n, k, commitment] = abiCoder.decode(["uint256", "uint256", "uint256[]"], encoded);
    ...
    decodeAddressKeyInitData(encodedData: string): {
        ...
        const AccountKeyAddressABI = ["function initialize(address signer) external"];
    ...
    decodeWebAuthnKeyInitData(encodedData: string): {
        ...
        const [credentialPubkey, credentialId] = abiCoder.decode(
            ...
            if (selector === SELECTORS.keyAddress) {
                ...
                keyInfoList.push({
                    ...
                    keyData: {
                        ...
                        credentialPubkey: decodedData.credentialPubkey,
                ...
                keyInfoList.push({
                    ...
                    keyData: {
                        ...
                        poseidonMerkleTreeDirectory: decodedData.poseidonMerkleTreeDirectory,
                    },
                });
            }
        }
    ...
    getEncodedZkOAuthRS256KeyInitData(zkOAuthRS256KeyData: ZkOAuthRS256KeyData): string {
        const commitment = zkOAuthRS256KeyData.commitment;
        const n = zkOAuthRS256KeyData.n;
        const k = zkOAuthRS256KeyData.k;
        const poseidonMerkleTreeDirectory = zkOAuthRS256KeyData.poseidonMerkleTreeDirectory;

        let abiCoder = ethers.AbiCoder.defaultAbiCoder();
        let encoded = abiCoder.encode(["uint256", "uint256", "uint256[]"], [n, k, commitment]);
        ...
        const contractInterface = new ethers.Interface(AccountKeyZkOAuthRS256Verifier3ABI);

        // 인코딩
        const encodedData = this.encodeCall(contractInterface, "initialize", [encoded, poseidonMerkleTreeDirectory]);
    ...
    getEncodedAddressKeyInitData(addressKeyData: AddressKeyData): string {
        ...
        const encodedData = this.encodeCall(contractInterface, "initialize", [addressKeyData.signerAddress]);

        return encodedData;
    }

    getEncodedWebAuthnKeyInitData(webAuthnKeyData: WebAuthnKeyData): string {
        // TODO: credentialPubkey 추가 등 검증 필요
        const AccountKeyWebAuthnABI = [
            "function initialize(bytes encoded, bytes32 rpIdHash, bytes memory origin) external",
        ];
        ...
        const encodedData = this.encodeCall(contractInterface, "initialize", [
            webAuthnKeyData.credentialId,
            webAuthnKeyData.rpIdHash,
            webAuthnKeyData.origin,
        ]);
    ...
    setEncodedKeyData(threshold: number, keyInfoList: KeyInfo[]): string {
        ...
        for (const keyInfo of keyInfoList) {
            ...
            if (keyInfo.keyType === PrimitiveAccountKeyTypes.keyZkOAuthRS256) {
                ...
                keyInitData = this.getEncodedZkOAuthRS256KeyInitData(zkOAuthRS256KeyData);
    ...
    setEncodedInitData(threshold: number, keyInfoList: KeyInfo[]): string {
        ...
        for (const keyInfo of keyInfoList) {
            ...
            if (keyInfo.keyType === PrimitiveAccountKeyTypes.keyZkOAuthRS256) {
                ...
                const AccountKeyAddressABI = ["function initialize(address owner, address signer) external"];
    ...
    getEncodedWebAuthnKey(credentialPubkey: string, credentialId: string, rpIdHash: string, origin: string): string {
        // TODO: 동작 검증 필요
        let pubkey = this.decodeCredentialPublicKey(ethers.getBytes(credentialPubkey));
        ...
        let encoded = abiCoder.encode(
            ["tuple(uint256,uint256,string)", "bytes32", "bytes"],
            [
                [ethers.hexlify(ethers.getBytes(x)), ethers.hexlify(ethers.getBytes(y)), credentialId],
    ...
    getEncodedOAuthKey(
        ...
        let encoded = abiCoder.encode(
            ...
            [[key.issuer, key.subToVerify, key.emailToVerify, key.verifyEmail, key.verifySub]]
        );
        return encoded;
    }
