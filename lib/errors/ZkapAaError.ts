import type { ZkapAaCode } from "./codes";
...
export interface ZkapAaErrorOptions {
  ...
  code: ZkapAaCode;
  /**
   * Throw-site identifier — by convention the throwing function name in snake_case
   * (e.g. `"submit_user_op"`). A free-form string, not a closed catalog: it is a
   * diagnostic/telemetry tag (stable across minification, unlike a stack trace), so
   * internal renames never break the public error type.
   */
  operation: string;
  message?: string;
  cause?: unknown;
